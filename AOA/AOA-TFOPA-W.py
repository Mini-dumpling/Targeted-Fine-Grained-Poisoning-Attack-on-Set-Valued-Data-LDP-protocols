...
def output_attack(epsilon: float, c: int, r_item: list, r_fre: list, n: int, S_dict):
    ...
    # u 的范围
    # for i in range(r):
    #     temp = r_fre[i] * (pt - pf) + pf
    #     print("temp:", temp)
    #     print("low:", round((temp * n - S_dict[r_item[i]]) / (1 - temp)))
    #     print("low:", round(S_dict[r_item[i]] / (r_fre[i] * (pt-pf) + pf) - n))
    #     print("up:", round((S_dict[r_item[i]] - n * (r_fre[i] * (pt-pf) + pf)) / (r_fre[i] * (pt-pf) + pf - 1)))
